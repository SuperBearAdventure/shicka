...
export default class RoadmapCommand extends Command {
	async execute(message, parameters) {
		...
		if (typeof channel !== "undefined") {
			await (await message.channel.send(`Before suggesting an idea in ${channel}, check the upcoming milestones of the game there:\nhttps://trello.com/b/3DPL9CwV/road-to-100`)).suppressEmbeds(true);
			return;
		}
		await (await message.channel.send("You can check the upcoming milestones of the game there:\nhttps://trello.com/b/3DPL9CwV/road-to-100")).suppressEmbeds(true);
	}
